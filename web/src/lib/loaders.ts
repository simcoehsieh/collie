...
export async function paneLoader({
  ...
  try {
    ...
    // FORK: THE POLL FOR A PAGE WHOSE LIVE FEED IS DOWN. With the stream up, a change is poked and
    // this read answers from what the bridge already holds; without it, the read is asked to WAIT for
    // a change (`?wait=`) so one request stands in for several and a change is answered as it lands.
    ...
    const read: PaneReadResponse = await (prefetched ??
      fetchPane(paneId, lines, scope, request?.signal, { wait }));
