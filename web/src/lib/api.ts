...
export interface FetchPaneOptions {
  ...
  wait?: number;
  /**
   * `false` leaves the pane's unseen mark alone: the read a finger starts on `pointerdown`
   * (lib/pane-prefetch.ts) may be the start of a scroll, not an open. Default true.
   */
  seen?: boolean;
}

/** Read one pane's mirror. */
export async function fetchPane(
  ...
  if (opts.seen ?? true) headers.set("x-collie-seen", "1");
  if (cached) headers.set("if-none-match", cached.etag);
