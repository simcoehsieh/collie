...
/**
 * FORK: no glide in the shipped app. The fork kept its own route entrance (`routeEnter` in
 * routes/root.tsx, drawn by skin.css) over upstream's `ScreenTransition` at 1.8.2, and the glide is
 * built on that deleted component: its `glideOwnsMove` is what stops the slide playing on top of the
 * glide, and nothing here asks `routeEnter` the same question. Off at this one gate, every caller
 * upstream wrote (`glideForward`, `glideBack`, `glideForwardWhenReady`) runs its plain `go()`, so
 * their call sites merge untouched. The engine's own tests still run it (vitest's MODE is "test").
 */
const FORK_GLIDE_OFF = import.meta.env.MODE !== "test";
...
export function canGlide(): boolean {
  if (FORK_GLIDE_OFF) return false;
