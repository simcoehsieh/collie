...
export function emittableKeys(block: Block): string[] | null {
  switch (block.kind) {
    ...
    case "multi-select":
      ...
      // FORK: plus `submitKeys` where the harness submits the whole set on one key from any option
      // row (agy), which is about the ADVANCE and rides alongside either toggle recipe.
