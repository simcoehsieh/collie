...
export type MultiSelectModel =
  | {
       ...
       *
       * FORK NOTE: orthogonal to `submitKeys` above, which is about the ADVANCE. A harness can
       * toggle by digit and still submit on one key from any row (agy); `submitKeys` is consulted
       * first by lib/multi-select-action.ts because it removes the advance walk entirely.
       */
