...
export function locateComposer(lines: StyledLine[]): ComposerBox | null {
  ...
  // FORK PATCH SUPERSEDED (v1.11.0). The fork's fix was "a blank row INSIDE the run is a paragraph
  // break the operator typed, not the end of the composer" — before it, any message carrying a
  // blank line made the composer invisible to the reply path, the verify read found nothing, and
  // the retry typed a second copy under the first (`codex--draft-blank-paragraph.txt` is that
  // screen). Upstream's starfield fix replaced the whole foreign-row predicate with `/^\S/`, which
  // a blank row does not match either — so the same case is covered by a broader rule, and taking
  // both would be two answers to one question. The fixture stays and still pins it.
