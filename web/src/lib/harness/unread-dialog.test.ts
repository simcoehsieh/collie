...
const CARD_FIXTURES = {
  ...
  // FORK: two captures upstream does not have (harness/agy/agy.test.ts, NEUTRAL). Both are modals
  // the agy grammars deliberately leave raw with the composer refused — the `?` shortcuts overlay
  // (no rule anchors it) and a multi-select's focused Write-in field — so the card is the right
  // answer on both: its Escape is the way out of either, which a flat refusal never offered.
  agy: { modals: ["agy--help-overlay.txt", "agy--multi-select-writein.txt"], notModals: [] },
  antigravity: { modals: ["agy--help-overlay.txt", "agy--multi-select-writein.txt"], notModals: [] },
