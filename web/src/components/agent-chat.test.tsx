...
describe("AgentChat — the pane header's identity block", () => {
  ...
  it("says the state in the DOT up here, and NOWHERE as a word, in every state", () => {
    ...
    for (const [status, word] of Object.entries(words) as [AgentStatus, string][]) {
      ...
      // FORK: minus the brand mark's own stylesheet, whose keyframe names (`cm-done`, `cm-blocked`)
      // carry these words as CSS identifiers. They are not text anybody reads; the claim is about
      // what the header DRAWS.
      const painted = Array.from(container.querySelectorAll("style")).map((el) => el.textContent ?? "");
      const visible = painted.reduce((all, css) => all.replace(css, ""), container.textContent ?? "");
...
describe("AgentChat — block-grammar scoping (an agent with no adapter)", () => {
  ...
  it("rides the actions belt's rule, below the statusline, always", () => {
    ...
    for (const text of [STATUS_TEXT, MENU_TEXT]) {
      ...
      // FORK: an ANCESTOR, not the parent — the fork's dock folds through a `Collapse`, which stands
      // between the composer and the block.
      const block = composer.closest('[data-slot="chrome-block"]')!;
      ...
      if (strip) {
        ...
        // FORK: the FIRST THING in the block is the dock's own `Collapse` (the fork folds the dock
        // away for a full-height mirror), and the composer is what stands inside it. The claim —
        // nothing of the fork's comes between the terminal and the belt — is the same one.
        expect(block.firstElementChild).toBe(composer.closest('[data-slot="collapse"]'));
      }
      cleanup();
    }
  });
