...
export function accentFor(agent: string | undefined | null): string | undefined {
  ...
  const brand = AGENT_BRANDS.get(canonicalAgent(agent.toLowerCase().trim()));
  // FORK: a brand may be ARTWORK (agent-icon-data.ts `ImageBrand`, which is how the fork draws
  // agy's own mark), and artwork carries no accent — the bar then tints with nothing, exactly as
  // it does for an agent it has never heard of.
  return brand === undefined || brand.kind === "image" ? undefined : brand.accent;
}
