...
function ConnectionStateBanner({
  ...
  const restarting = shownTone === "red" && probe === "gateway";
  const needsSignIn = shownTone === "red" && probe === "auth";
  ...
  return (
    ...
    >
      <div className="min-h-0 overflow-hidden">
        ...
        >
          {restarting ? (
            <Loader2 className={cn("size-3.5 shrink-0 animate-spin", view.icon)} />
          ) : (
          ...
          {/* Actions only in red — amber is ambient (no buttons), green is a passing confirmation.
              FORK: and not while the bridge is restarting, which needs no tap. */}
          {shownTone === "red" && !restarting && (
            <>
              <Button
                size="sm"
