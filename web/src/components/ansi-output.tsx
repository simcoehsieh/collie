...
export const AnsiOutput = memo(function AnsiOutput({
  ...
  // The blocks: the model's when a caller built one (the fork's parse-once path), else the ones the
  // caller already built for its card dock (upstream, .adr/0059), else built here from our own lines.
  // Skipping the parse is the point in both cases: AgentChat's polling path pays for one parse per
  // screen, not two. `grammars` and `nativeMirror` only reach the last branch; a caller that built
  // the blocks itself already applied both there (components/agent-chat.tsx).
  const blocks = useMemo(
    () =>
      model !== undefined
        ? model.blocks
        : (builtBlocks ?? buildBlocks(ownLines, { agent, grammars, nativeMirror })),
    [model, builtBlocks, ownLines, agent, grammars, nativeMirror],
  );
