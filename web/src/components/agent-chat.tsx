...
type Drawer =
  ...
  | "handoff"
  // Upstream 1.9.0: this pane's own settings sheet (components/pane-settings-sheet.tsx).
  | "paneSettings"
  | null;
            ...
            >
              ...
              >
                ...
                <div className="flex h-3 min-w-0 items-baseline gap-2">
                  ...
                  {/* FORK: which model this pane's agent is on, and at what effort (read bridge-side off
                      its own log, bridge/session-facts.ts). It rode the composer's status band until
                      1.9.0 removed that band; this path line is where the pane's other read-only meta
                      now lives, so it moved here rather than going. Absent on a pane whose log the
                      bridge has not read, and the line is then byte-identical to upstream's. */}
                  {modelLabel({ model: agent.model, effort: agent.effort }) !== null && (
                    <span
                      data-slot="header-model"
                      className="min-w-0 shrink truncate pl-2 font-mono text-[11px] leading-3 text-muted-foreground/80"
                    >
          ...
          <Collapse open={!zen}>
            <div className="relative shrink-0">
              ...
              <div data-slot="chrome-block" className="border-t border-rule bg-chrome">
                {/* THE RESTORE DOOR, and nothing else any more. 1.9.0 moved the switcher onto the
                    actions belt (a 0px mark at its right end, `pullHandle` above), so a full-width
                    30px strip whose only job was "open the switcher" would now be the second way to
                    do one thing. What it still owes the operator is the way BACK from the fork's
                    folded dock: with the dock folded there is no belt on screen, so the fold would be
                    a one-way door. Drawn only then — and it keeps the drag ref, so the same gesture
                    that folded the dock reopens it. */}
                <Collapse open={!composing && !dockOpen}>
                  <button
                    type="button"
                    aria-label={t("chat.dock.showAria")}
                    aria-expanded={false}
                    ref={sheetPull.ref}
                ...
                <Composer
                  ...
                  agent={agent?.agent}
                  isShell={isShell}
                  // FORK: the state still goes down, but NOTHING DRAWS IT — see the prop's own note
                  // in composer.tsx: "stop and edit" and the send queue branch on it.
                  status={agent?.status}
                  // NO `stale` GOES DOWN ANY MORE. The composer drew the state as a
