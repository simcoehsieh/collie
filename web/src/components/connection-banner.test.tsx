...
import { ConnectionBanner, GREEN_MS, REPROBE_MAX_MS, REPROBE_MIN_MS } from "./connection-banner";
...
describe("ConnectionBanner — the single connection surface", () => {
  ...
  it("says the bridge is restarting on a gateway answer, drops Retry, and re-probes on a doubling timer", async () => {
    ...
    renderBanner();
    await act(async () => {});
    expect(announced("alert")).toHaveTextContent("Collie is restarting…");
  ...
  it("dates the red row when the data on screen came from the cache", async () => {
    ...
    await act(async () => {});
    expect(announced("alert")).toHaveTextContent(/Can't reach Collie through the tunnel — last seen \d/);
  });
