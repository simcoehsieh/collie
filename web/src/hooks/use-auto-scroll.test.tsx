...
describe("useAutoScroll — resize-induced scroll is not a user scroll (#155)", () => {
  ...
  it("keeps following when a docked card leaves and the container grows back (ADR 0059)", async () => {
    ...
    fireEvent.scroll(el);
    // FORK: scroll reads are coalesced per frame, so a second event inside the first one's frame is
    // read at the frame's end — where it is still recognised as layout and re-pinned.
    await flushFrame();
    expect(onAtBottomChange).not.toHaveBeenCalledWith(false);
    expect(el.scrollTo).toHaveBeenCalledWith({ top: 500, behavior: "auto" });
  });
