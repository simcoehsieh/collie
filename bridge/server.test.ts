...
describe("serveStatic — a text file ships compressed", () => {
  ...
  test("index.html goes out as built at the root, and resolved to the mount under a path", async () => {
    ...
    expect(await (await serveStatic("/", null, null, dir)).text()).toBe(SHELL);

    const mounted = await (await serveStatic("/space/w1", null, null, dir, "/collie/")).text();
  ...
  // FORK: this fork serves static files through its own brotli + ETag path (http-cache.ts) and has
  // no gzip cache, so upstream's cache-entry count is not asserted. A mounted shell is compressed per
  // request with no cache at all, which keeps two mounts apart by construction; what is still pinned
  // is that the compressed body is the MOUNTED one, for each mount.
  test("a gzipped shell is the mounted one, for each mount", async () => {
    const { dir } = await distTree();
    ...
    const read = async (base: string) => {
      const res = await serveStatic("/", "gzip", null, dir, base);
      expect(res.headers.get("content-encoding")).toBe("gzip");
