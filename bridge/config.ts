...
export interface Config {
  ...
  /**
   * FORK: where the operator's per-pane notification rules live — `notify.toml`, a sibling in the
   * same dir, read the same way (bridge/operator-notify.ts) and likewise never read here.
   */
  notifyFile: string;
  /**
   * Where the operator's prompt-cache overrides live — `cache-rules.toml`, another file on the same
...
export function loadConfig(env: Environment = process.env): Config {
  ...
  return {
    ...
    shotHosts: envList("COLLIE_SHOT_HOSTS", env),
