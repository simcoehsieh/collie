...
| Notification rules in a file: `notify.toml` as the sixth operator file | new `bridge/operator-notify.ts` (+test), `notify.toml.example`; hunks in `bridge/config.ts` (`notifyFile`), `bridge/notify-prefs.ts` (an optional reader, `refreshOperatorRules`, `operatorPanes` on the view), `bridge/index.ts` (reader + timer), `bridge/server.ts` (one refresh before the GET), `cli/doctor.ts` (one finding), `components/notify-prefs-control.tsx` (which rule applies and whose it is), `web/src/lib/types.ts`, seven locales, pins in `solo-baseline` (config keys) and `server.test` (`cfg()`) | The file speaks only where the phone said nothing, through the same `ruleFor` and the same row grammar (`coercePaneRule`) — if upstream grows a per-pane rule store, keep the file and point it at theirs |
| The app icon badge: outstanding alerts on the icon | new `web/src/lib/app-badge.ts` (+test), `hooks/use-app-badge.ts` (+test); hunks in `bridge/notifications.ts` (`count` on the summary, `badge` on the message and `badge: 0` on a clear), `bridge/push.ts` (one field), `web/src/lib/push-decision.ts` (the field rides through), `web/src/sw.ts` (`applyBadge` after show/clear), `routes/root.tsx` (one hook call), four pins in `notifications.test` and one in `push-decision.test` | The worker sets, the page corrects (iOS gets no retraction): if upstream badges, keep the page-side correction — it is what clears the dot after a read |
| Which model, at what effort: two facts off the agent's own log | new `bridge/session-facts.ts` (+test), `web/src/lib/model-label.ts` (+test); hunks in `bridge/journal/types.ts` (`SessionFacts`, optional `facts` on the adapter), `journal/claude.ts` + `journal/codex.ts` (one pure reader each, one line in the factory), `journal/files.ts` (`tail`), `bridge/server.ts` (one store, four lines in `withActivity`), `bridge/types.ts` + `web/src/lib/types.ts` (`model`, `effort`), `components/agent-card.tsx` (`ModelLine`), `components/composer.tsx` (two props, one run on the band), `components/agent-chat.tsx` (two props), pins in both `solo-baseline` key lists | Display-only on the status line's terms: text, no branch, no sort. Hold-last-good in the store — a tail with no assistant row keeps the previous answer — so the chip never flickers while a tool result is being written. If upstream carries a model on the pane, drop `session-facts.ts` and point `model-label.ts` at their field |
