...
function withGitBuildFixture<T>(
  ...
  try {
    ...
    // FORK: vite.config.ts imports ./branding (the operator's icon and name overlay), which in turn
    // imports src/lib/json. Upstream's fixture copies only the config and vite-icons, so without
    // these two the identity host cannot resolve the module and the build fails before it starts.
    copyFileSync(join(sourceRoot, "web", "branding.ts"), join(web, "branding.ts"));
    mkdirSync(join(web, "src", "lib"), { recursive: true });
    copyFileSync(join(sourceRoot, "web", "src", "lib", "json.ts"), join(web, "src", "lib", "json.ts"));
