...
- **Stop, and edit what you just sent.** While the agent is working on the last message, a row above the input offers one tap that sends Esc to the pane and puts the message back in the box — above anything typed since — to change and send again. Offered only while working, only for the message that put the agent to work, and dropped by any outcome that already restores the draft. (components/composer.tsx)
