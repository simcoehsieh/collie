...
- **A Codex message with a blank line in it sends.** The Codex composer locator walked up from the status row and stopped at the first blank row, and Codex paints an empty draft line as one space — so any message carrying a paragraph break (a list, then a paragraph) left the composer unlocated: the text landed, the phone reported "Message didn't reach the input box", pressed no Enter, and a retry typed a second copy under the first. A blank row inside the draft is now a paragraph break; a foreign row still ends the walk. (web/src/lib/harness/codex/chrome.ts, fixture `codex--draft-blank-paragraph.txt`)
- **Each pane says which model it is on, and at what effort.** The bridge reads the pair off the newest turn of the agent's own session log — Claude Code's `message.model` and the row's `effort`, Codex's `turn_context` — through a read-through cache like the status line's (a stat per refresh, a tail read only when the log grew), and the snapshot carries them as two text fields. The herd row shows `fable-5-1 · xhigh` under the address line; the composer's status band shows it between the machine and the state. Nothing branches on either. (bridge/session-facts.ts, `facts` on the Claude and Codex journal adapters, web/src/lib/model-label.ts)
- **The app icon shows a badge.** iOS never badges a web app by itself; the service worker now asks for the dot with the number of alerts the bridge says are outstanding, and the app clears or corrects it from the herd whenever it is open — blocked panes plus finished-and-unread ones. (bridge/notifications.ts `count`, `badge` on the push, sw.ts, hooks/use-app-badge.ts)
- **Per-pane notification rules can live in a file.** `notify.toml` beside `.env` — the sixth operator file, read live like the others — carries the rules that are part of the install ("the listener panes never buzz for a finished job"), so they can sit in a repo and follow the setup to the next machine. The phone's own rules win for the same pane; Settings shows which rule applies and names the file when it is the file's. `collie doctor` reports the file. (bridge/operator-notify.ts, notify.toml.example)
